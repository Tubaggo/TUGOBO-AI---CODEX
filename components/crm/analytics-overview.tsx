...
export function AnalyticsOverview() {
  const { copy, formatIntent } = getCrmI18n();
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{copy.dashboard.eyebrow}</p>
        <h1 className="mt-1 text-2xl font-semibold text-slate-950">{copy.dashboard.title}</h1>
        <p className="mt-2 max-w-3xl text-sm text-slate-500">{copy.dashboard.description}</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {analytics.stats.map((stat) => (
          <div key={stat.key} className="rounded-2xl border border-slate-200 bg-white p-5">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">
              {copy.dashboard.stats[stat.key].label}
            </p>
            <p className="mt-3 text-2xl font-semibold text-slate-950">{stat.value}</p>
            <p className="mt-2 text-sm text-slate-500">{copy.dashboard.stats[stat.key].note}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h2 className="text-sm font-semibold text-slate-900">{copy.dashboard.channelDistribution}</h2>
              ...
              <div key={item.channel}>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-slate-600">
                    {item.count} {copy.dashboard.leadsCount}
                  </span>
                </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h2 className="text-sm font-semibold text-slate-900">{copy.dashboard.leadFunnel}</h2>
          <div className="mt-5 space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-900">{copy.dashboard.recentLeads}</h2>
            <Link className="text-sm text-slate-600 underline" href="/leads">
              {copy.common.viewLeads}
            </Link>
          </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {item.assignedUser?.fullName ?? copy.common.unassigned} · {item.lead.estimatedValue ?? "-"}{" "}
                      {item.lead.currency}
                    </p>
                  </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-900">{copy.dashboard.recentConversations}</h2>
            <Link className="text-sm text-slate-600 underline" href="/conversations">
              {copy.common.openInbox}
            </Link>
          </div>
          <div className="mt-4 space-y-3">
              ...
              <div key={item.conversation.id} className="rounded-2xl bg-slate-50 px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">{formatIntent(item.conversation.intent)}</p>
                  </div>
